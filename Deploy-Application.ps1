...
Try {
	...
	[string]$appName = 'Zoom'
	[string]$appVersion = '5.11.1.6602'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.1'
	[string]$appScriptDate = '2022.07.15'
	...
	$PrereqApp1Name = "Zoom Outlook Plugin"
	# Prerequisite application 1 install path.
	$PrereqApp1InstallPath = "$PSScriptRoot\Files\ZoomOutlookPluginSetup.msi"
	# Prerequisite application 1 install parameters.
	$PrereqApp1InstallParam = "/quiet /qn /norestart /log install.log"
	...
	$64bitAppInstallName = "Zoom"
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\ZoomInstallerFull.msi"
	# Application  install parameters.
	$64bitAppInstallParam = "/qn" 
	...
	$64bitAppUninstallPath = "$PSScriptRoot\Files\CleanZoom.exe"
	# Application uninstall parameters.
	$64bitAppUninstallParam = "/qn /quiet /silent /norestart /q /s"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$RemoveAppNamesMSICheck = Get-InstalledApplication -Name "$PrereqApp1Name"
		If ($null -ne $RemoveAppNamesMSICheck) {
			...
			Catch [System.Exception] {
				Write-Log -Message "Uninstaling previous MSI versions of $PrereqApp1Name failed with error: $_."
		...
		# Else, log results from check.
		Else {
			Write-Log -Message "Previous MSI versions of $PrereqApp1Name are not installed."
		}
		...
		If ($null -eq $RemovePrereqAppNamesCheck1) {
			Try {
				...
				Execute-MSI -Action Uninstall -Path "$PrereqApp1InstallPath" -Parameters "$PrereqApp1InstallParam"
